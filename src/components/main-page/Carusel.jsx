...
export const Carusel = ({ caruselImg }) => {
  ...
  const endTouch = e => {
    ...
    } else if (x1 < x2 && position < 0) setPosition(position + 164);
  };

  const arrowLeftHandler = () => { (position < 0) && setPosition(position + 164) };
  const arrowRightHandler = () => { (position <= 0 && position > rightSizeMax) && setPosition(position - 164) };
